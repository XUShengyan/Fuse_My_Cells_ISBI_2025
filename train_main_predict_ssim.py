...
from train.trainer_pred_ssim import Trainer
# from train.trainer_simple import Trainer

def run_training(config_path, config_name, task_name='Test'):
    """
    Load configuration from file path and name, initialize model, data loaders, and trainer, then start training.
    """
    ...
    if model_path:
        if not os.path.isfile(model_path):
            ...
            try:
                ...
                # Load full training state (requires corresponding save code modification)
                # checkpoint = torch.load(model_path, map_location=device)
...
if __name__ == '__main__':
    # Determine parameter input method based on the operating system
    if platform.system() == 'Windows':
        config_path = r"D:\PythonProjects\25_Fuse_my_cells\configs"
        # config_path = r"C:\PythonProjects\25_Fuse_my_cells\configs"
        # config_path = r"D:\OneDrive_Family\OneDrive\CodeLib\Python\25_Fuse_my_cells\configs"

        config_name = r"pred_ssim_4060.yaml"

        # task_name = 'Unet_trans_gated_4L_128_nssim_ssim'
        task_name = 'SSIM_predictor'
        # task_name = 'Test'
